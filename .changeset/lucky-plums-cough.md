...
- Keep the read bound on a non-paginated index-union `findMany` when the table
  has RLS enabled, when the `where` carries a filter Convex cannot evaluate such
  as `contains`, or when it filters through a relation. All three used to cancel
  the bound entirely and read every row in each probed range, so
...
- Rows and their order are unchanged. `ne`, `notIn` and `isNotNull` combined with
  an `orderBy` no index can serve still read their whole complement range; that
  shape is unaffected.
