...
- GitHub issue sync: N/A: user preference forbids PR creation, so there is no PR
  to reference; no issue comment will be posted without user instruction.
...
- next owner: none — user owns the commit/PR decision
...
| Exact per-PR task ownership | no | N/A: user preference forbids creating a PR, so this plan owns a no-PR slice |
...
| Branch decision for code-changing task | yes | Already on `issue-408`, a dedicated non-`main` branch |
...
| Commit / PR expectation decision | no | N/A: explicit standing user preference — "Do not create PR under any circumstances, unless user prompts to." Recorded as user decline, not as a missing ask. |
| Task-style PR body decision | no | N/A: no PR created |
| Task-plan PR body evidence | no | N/A: no PR created |
| GitHub issue sync expectation decision | no | N/A: no PR to reference; no comment posted without user instruction |
...
| Exact per-PR task ownership | no | Record the exact PR and dedicated plan, or the not-yet-created single-PR slice | N/A: user preference forbids PR creation |
...
| Commit created | no | For verified code-changing work, stage the entire current checkout per repo policy and create a commit; N/A only for no local patch, explicit user decline, analytical/blocked/inconclusive work, or recorded external blocker | N/A: explicit standing user decline — "Do not create PR under any circumstances, unless user prompts to." Work left staged in the working tree on branch `issue-408`. |
| PR create or update | no | For verified code-changing work, run `check`, push, create or update the PR, and sync PR body to the task-style final handoff; N/A only for no local patch, explicit user decline, analytical/blocked/inconclusive work, or recorded external blocker | N/A: explicit standing user decline |
...
| GitHub issue sync-back | no | Post concise issue sync after PR exists, or record N/A/blocker | N/A: no PR to reference and no user instruction to comment |
...
| Commit / PR / GitHub sync | skipped | N/A: explicit standing user decline of PR creation | closeout |
...
- Commit line: ➖ N/A — user standing preference forbids PR creation; changes
  left uncommitted in the working tree on branch `issue-408`.
- PR line: ➖ N/A — same.
- Issue line: 🐛 Fixes #408 (not synced to GitHub; no PR to reference).
...
Final handoff / sync:
- Commit: N/A: explicit standing user decline of PR creation; work left in the
  working tree on branch `issue-408`.
- PR: N/A: same.
- Issue: N/A: no PR to reference; no comment posted without user instruction.
