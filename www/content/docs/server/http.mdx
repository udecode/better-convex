...
`queryOptions` ships a 30 second `staleTime`, so an RSC-prefetched entry hydrates without an immediate refetch on mount. `refetchOnMount` stays at TanStack's default, so a route invalidated while unmounted still refetches when it comes back. Override either per call:
...
`queryKey()` returns the exact key an entry is stored under — the same key `queryOptions()` and RSC prefetching use. Pass it to `getQueryData`/`setQueryData`, which match by exact key.
...
crpc.http.todos.get.queryKey({ params: { id: todoId } });
// => ['httpQuery', 'todos.get', { params: { id: todoId } }]
```

Reach for `queryFilter()` instead when you want every args variant of a route.
...
| `queryKey` | `(args?)` | Exact cache key for these args — for `getQueryData`/`setQueryData` |
| `queryFilter` | `(args?, filters?)` | Filter for `invalidateQueries`, etc. Omit `args` to match every variant |
