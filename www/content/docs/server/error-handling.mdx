...
A value that fails the [`.output()` schema](/docs/server/procedures#output-validation) throws `INTERNAL_SERVER_ERROR` with message `Output validation failed`. The Zod issues arrive in `error.data.ZodError`, naming the expected type and the path that failed:
