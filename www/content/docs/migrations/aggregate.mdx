...
| `min(ctx, { namespace })` | `rank(...).min()` | First row in rank order |
| `max(ctx, { namespace })` | `rank(...).max()` | Last row in rank order |
...
- `CLEARING` indexes (rebuild or prune in progress) finish draining first
