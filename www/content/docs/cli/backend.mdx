...
`<functionsDir>/generated/server.ts` is rewritten from your schema before codegen
reads any of your modules, so a copy left behind by an older kitcn — or one that
no longer matches the schema — is repaired in place rather than breaking the run.
