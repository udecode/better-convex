...
Sync mode throws if the transaction exceeds `mutationMaxRows`. That budget covers the matched rows plus every row read by foreign-key cascade fan-out in the same transaction.
...
- `mutationMaxRows`: `10000` (row budget for one transaction, shared by matched rows and cascade fan-out)
