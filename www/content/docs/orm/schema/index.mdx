...
export default defineSchema({ users, posts }, {
  ...
  defaults: {
    ...
    mutationMaxRows: 10000,        // row budget per transaction
...
- `defaults.mutationMaxRows` -- row budget for a single `update()`/`delete()` transaction, shared by the matched rows and every row read by foreign-key cascade fan-out. Built-in fallback: `10000`.
...
| Update/delete transaction row budget exceeds `mutationMaxRows` | sync mode throws; async mode schedules continuation | sync mode throws; async mode schedules continuation | sync mode throws; async mode schedules continuation |
