...
`limit` and `orderBy` are pushed into the relation index when the index already
walks in that order — after the foreign key, `index('by_user').on(t.userId)`
orders by creation time, so the query above reads five posts per user. Sorting
by any other column reads the parent's whole child partition and sorts it in
memory; add that column to the relation index (`index('by_user_rank').on(t.userId, t.rank)`)
