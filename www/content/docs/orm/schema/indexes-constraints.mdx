...
Async soft cascade requires an exact child index whose declared fields are the referencing columns. For example, `index('by_author').on(t.authorId)` qualifies; `index('by_author_rank').on(t.authorId, t.rank)` does not. Mutable trailing fields cannot provide stable continuation across scheduled mutations.
</Callout>
