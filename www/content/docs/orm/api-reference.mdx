...
`import()`, so `kitcn/orm` does not reach them on its own — register the ones you use:
...
|---|---|
| `aggregateCapability()` | `aggregateIndex`/`rankIndex` schemas, filtered `count()`, `count({ select })`, `aggregate()`, `groupBy()`, `rank()`, relation `_count`, and `orm.api()` backfill handlers |
| `migrationCapability()` | `orm.api()` migration handlers |

Unfiltered `count()` is served by the native Convex count syscall and needs no capability.
