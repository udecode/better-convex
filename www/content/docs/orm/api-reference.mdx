...
- `mutationMaxRows` (default `10000`) is the row budget for the whole transaction: matched rows and foreign-key cascade fan-out draw on the same budget. Rows are collected in `mutationBatchSize` pages (default `400`).
...
- Use `execute({ mode: 'sync' })` to force all rows in a single transaction (throws when the transaction exceeds `mutationMaxRows`).
