...
`kitcn codegen` writes this for you. Convex bundles everything a module statically imports and has
no dynamic `import()`, so `kitcn/orm` never reaches the aggregate runtime on its own — without the
capability, `aggregateIndex`/`rankIndex` schemas and filtered `count()` / `aggregate()` / `groupBy()`
/ `rank()` / relation `_count` throw a setup error. Unfiltered `count()` needs nothing.
</Callout>
