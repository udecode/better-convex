...
| `COUNT_FILTER_UNSUPPORTED` | Filter uses unsupported operators (`NOT`, string, relation, or non-finite `OR`), or a range filter scans more than `aggregateWorkBudget` buckets |
