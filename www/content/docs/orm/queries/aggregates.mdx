...
Automatic pruning follows aggregate lifecycle state, so kickoff reads stay bounded as backing tables grow. To recover state-less storage, invoke `generated/server:aggregateBackfill` in `prune` mode with exact `tableName` and `indexName` arguments.
