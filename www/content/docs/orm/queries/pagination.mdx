...
`where: { id: { in: [...] } }` pages in the order the ids are given, at one document read per row returned. `maxScan` caps that too. Adding `orderBy` switches the read to a scan, so the order has to be one a scan produces: a single field that an index leads with, with no `withIndex(...)` pinned. `orderBy: { createdAt }` reads every id in the list to sort it, so it cannot be capped at all.
...
- `maxScan` with `where: { id: { in: [...] } }` and an `orderBy` no scan produces: `createdAt`, more than one field, a field without a leading index, or a pinned `withIndex(...)`.
