...
`queryFilter` builds a filter for `invalidateQueries`, `cancelQueries`, `removeQueries`, and friends. Omit `args` to match every args variant of the function:

```tsx showLineNumbers {2,5,8}
// Every cached variant of user.list
queryClient.invalidateQueries(crpc.user.list.queryFilter());

// Only variants whose args contain `{ status: 'active' }`
queryClient.invalidateQueries(crpc.user.list.queryFilter({ status: 'active' }));

// Combine with any other TanStack filter option
...
<Callout icon={<InfoIcon />}>
**`queryKey` vs `queryFilter`:** `queryKey(args)` is the exact key an entry is stored under — use it for `getQueryData`/`setQueryData`. `queryFilter(args?)` matches on prefix, so partial args match every query whose args contain them.
</Callout>
