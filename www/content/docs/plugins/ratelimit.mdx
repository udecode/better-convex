...
Add `--prod` when targeting production. Each call deletes at most 500 rows by
...
Protection state is a bounded in-memory LRU. It evicts failure histories before
blocks and refreshes blocks that keep sending requests. Under more than 4,096
simultaneous blocked values per prefix, the coldest block can be evicted; that
value falls through to the normal database-backed rate limiter.
