...
The ephemeral block cache is an in-memory `Map<string, number>` that caches "blocked until" timestamps. When a shard fails, later calls skip its database read until the block expires and try another shard. A request is cache-blocked only when every shard it could use is blocked. The cache is per-`Ratelimit` instance and resets on each Convex function invocation. Pass `ephemeralCache: false` to disable it, or pass a shared `Map` across multiple `Ratelimit` instances to share the cache.
