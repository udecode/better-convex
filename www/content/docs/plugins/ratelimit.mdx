...
| `maxReserved` | `number` | `undefined` | Maximum tokens a request can "borrow" from future capacity. Dealt across shards as whole-token headroom. Only applies to `fixedWindow` and `tokenBucket`. Not supported by `slidingWindow`. |
...
Return the remaining tokens, reset time, and limit for an identifier. It reads every raw shard, evaluates them at one common timestamp, and then sums them, so the count is exact and a full shard cannot absorb another shard's refill.
...
| `reset` | `number` | Epoch ms when tokens will be available. Denied reserved fixed-window and token-bucket requests use the point where the request fits within `maxReserved`, not full debt recovery. |
...
| `state` | `RatelimitState` | Projected aggregate state. Sliding windows retain current and previous counts with both timestamps so later projections preserve boundary decay. |
...
`getValue()` reports tokens left, while `calculateRatelimit()` takes stored state — the two differ for sliding windows, which store current and previous used counts. The snapshot retains that projected aggregate state, and `snapshotToState` returns it for later calculations:
...
- The limiter tries its preferred shard candidates first, then retries untouched shards before denying a request. Extra reads happen only when the preferred candidates cannot serve it.

For most use cases, `shards: 1` (the default) is fine. Increase shards only when you see write contention on hot identifiers.

#### Ephemeral cache

The ephemeral block cache is an in-memory `Map<string, number>` that caches "blocked until" timestamps per shard, requested count, and reservation mode. When a shard fails, equivalent calls skip its database read until the block expires and try another shard; smaller or reserved requests still probe it. A request is cache-blocked only when every shard it could use is blocked, and `reset` is the earliest retry across cached and freshly evaluated shards. The cache is per-`Ratelimit` instance and resets on each Convex function invocation. Pass `ephemeralCache: false` to disable it, or pass a shared `Map` across multiple `Ratelimit` instances to share the cache.
