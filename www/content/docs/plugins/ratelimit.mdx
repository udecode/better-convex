...
| `options.shards` | `number` | Write distribution shards (default `1`). The budget is split evenly across shards. |
...
Return the remaining tokens, reset time, and limit for an identifier. It reads every shard and sums them, so the count is exact.
...
| `remaining` | `number` | Tokens left after this request (floored to 0). With `shards > 1` this is extrapolated from the shard that served the request, so it is an estimate — use `getRemaining()` for an exact count. |
...
| Field | Type | Description |
| --- | --- | --- |
| `value` | `number` | Tokens left at read time across the whole limit. Sampled shards are summed, then scaled up by the share of the budget they cover |
...
`getValue()` reports tokens left, while `calculateRatelimit()` takes stored state — the two differ for sliding windows, which store the used count. `snapshotToState` converts between them:
...
The shares add back up to the budget you configured, so `fixedWindow(20, '1 m', { shards: 8 })` still grants 20 requests per minute. Budgets that do not divide evenly hand the remainder to the low-numbered shards — `limit: 5` over two shards becomes `3` and `2`, not `2.5` each, because a shard can only spend whole tokens.
...
The ephemeral block cache is an in-memory `Map<string, number>` that caches "blocked until" timestamps. When a `limit()` call fails, later calls routed to the same shard skip the database read entirely until the block expires. Entries are per shard, so an exhausted shard never blocks the ones that still hold tokens. The cache is per-`Ratelimit` instance and resets on each Convex function invocation. Pass `ephemeralCache: false` to disable it, or pass a shared `Map` across multiple `Ratelimit` instances to share the cache.
