...
| --- | --- | --- | --- |
| `shards` | `number` | `1` | Number of shards for write distribution. The configured budget is split evenly across shards, so the total limit stays the same. Higher values reduce contention at the cost of less precise counts (see [Sharding](#sharding)). |
...
| `remaining` | `number` | Tokens left after this request (floored to 0). With `shards > 1` this is extrapolated from the shard that served the request. |
...
| Field | Type | Description |
| --- | --- | --- |
| `value` | `number` | Tokens left at read time, scaled to the full limit when `shards > 1` |
...
When `shards > 1`, the configured budget is divided evenly across shards and each `limit()` call picks a random shard (or two, using power-of-two-choices when `shards >= 3`) to reduce write contention. Total throughput stays at the configured limit.

Two trade-offs come with it:

- Reads (`check`, `getRemaining`, `getValue`) sample a subset of shards and scale the result back up, so remaining counts are approximate.
- A single call spends from one shard, so `count` can never exceed `limit / shards`. Keep the per-shard share comfortably above your largest `count` — ten or more is a good target. The algorithm builders throw when `limit / shards` (or `maxTokens / shards` for `tokenBucket`) drops below `1`, since every request would otherwise be denied.
