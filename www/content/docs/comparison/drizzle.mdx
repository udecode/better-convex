...
- Update/delete transactions get a `mutationMaxRows` row budget (default `10000`) shared by matched rows and cascade fan-out, collected in `mutationBatchSize` pages (default `400`)
...
| `update()`/`delete()` exceeds row budget | Matched rows plus cascade fan-out > `mutationMaxRows` | Narrow filters, increase `defaults.mutationMaxRows`, or switch to paged mutation execution |
