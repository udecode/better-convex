...
```ts title="convex/functions/schema.ts" showLineNumbers {1-2,4-68}
import {
  convexTable,
  defineSchema,
  id,
...
- Use `authAction` from `convex/lib/crpc-action.ts` for Better Auth endpoints that can run external plugin work such as Stripe, Polar, or direct email delivery. Convex mutations cannot call those SDKs.
