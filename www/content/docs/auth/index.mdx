...
In queries and mutations — including the shared cRPC auth middleware — read the session row
directly:
...
Reach for `getAuth(ctx)` in the modules that call `auth.api.*` — actions, HTTP routes, and
organization/admin mutations. The generated runtime handles adapter selection:
...
**Bundle:** Convex has no dynamic `import()`, so every module that names `getAuth` carries your
whole Better Auth definition and all its plugins in its evaluation closure. Keep it out of
`convex/lib/crpc.ts`, which every procedure module imports. Authenticated actions build from
`convex/lib/crpc-action.ts`, the one builder module that imports it.
</Callout>
